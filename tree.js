(function (ns) {
    ...
    ns.xmasTree = function (canvas, left, top, height) {
        ...
        function createSprites() {
            for (k = 1; k <= 10; k++) {
                ...
                for (i = 0; i < 446; i++) {
                    ...
                    var y = Math.random() * 2 - 1; // other random number from -1 to 1
                    var B = Math.sqrt((x * x + y * y) - x / .9 - 1.5 * y + 1); // This is something similar to a radial gradient.
                    var R = 67 * (B + 1) * (L = k / 9 + .8) >> 1; // Also for the colors of the leaves
        ...
        function createTree(height) {
            vectors = [];
            for (var k = 0; k < 100; k++) // 200 branches
            {
                var x = 0,
                ...
                {
                    P = 5; // some parameter to control the distance between sprites
                    ...
                    vectors[numberOfVectors++] = [
                        ...
                        j / H * 20 + ((j += 16) > H & Math.random() > .8 ? Math.random(P = 9) * 4 : 0) >> 1];
        ...
        this.draw = function (context) {
            //context.clearRect(0, 0, 446, 446); // clear the canvas
            context.fillStyle = '#cca';
            var mcd = Math.cos(D),
                msd = Math.sin(D);
            for (i = 0; L = vectors[i++];) // for all the ordered vectors
            {
                context.drawImage(spriteArray[L[3] + 1], left + L[0] * mcd + L[2] * msd >> 0, top + L[1] >> 1);
            }
